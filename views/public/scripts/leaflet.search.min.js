$(document).ready(function () {
  ...
  $("#filterSearchButton").click(function(){
    ...
    if (!filterArray.length && !filterShape && !filterType) { 
      $("#filterError").show();
      $("#filterClear").hide();
      return;
    }
  ...
  function testSearch(){
    ...
    for(var prop in control._layers) {
      if (control._layers[prop].name.indexOf(compareString) != -1) {
        map.addLayer(control._layers[prop].layer)
        indexes.push(prop)
      }
  ...
  function createFilteredList(featureList){
    ...
    for (var index in featureList){
      var row = $("<tr>", {value: index}).append($("<td>", {text: featureList[index].feature.properties.gid}))
