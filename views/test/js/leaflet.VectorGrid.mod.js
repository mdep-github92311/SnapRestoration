...
L.VectorGrid = L.GridLayer.extend({
			...
			for (var layerName in vectorTile.layers) {
				...
				var layer = vectorTile.layers[layerName];
			 	...
			 	for (var i = 0; i < vectorTile.layers[layerName].length; i++) {
					...
					var featureStyle = this.getFeatureStyle(feat, layerName);

					var geojson = feat.toGeoJSON(coords.x, coords.y, coords.z);

					for (var j = 0; j < featureStyle.length; j++) {
						var style = L.extend({}, L.Path.prototype.options, featureStyle[j]);
						this.mapLayer.options = style;
						this.mapLayer.addData(geojson);
					}
				}
			}

			if (this._map != null) {
				this.mapLayer.addTo(this._map);
			}
	...
	getFeatureStyle: function(feat, layerName){
		var styleOptions = this.options.vectorTileLayerStyles[layerName] || L.Path.prototype.options;
		...
		if (styleOptions instanceof Function) {
			styleOptions = styleOptions(feat.properties, this._map.zoom);
		}
...
// 		this._slicer = geojsonvt(geojson, options);
	...
	_getVectorTilePromise: function(coords) {
		...
		var tile = new iDB('tileDB').getTile(this._tileCoordsToKey(coords), this.options.name);

		return tile.then(function(pureTile) {
						var pbf_data = pako.inflate(new Uint8Array(pureTile.purePBF));

						var pbf = new index(pbf_data);

						return new VectorTile(pbf)
		}).catch(function(e){
			console.log('catch: ' + e);

			var tileUrl = L.Util.template(thisVG._url, L.extend(data, thisVG.options));

			return fetch(tileUrl, thisVG.options.fetchOptions)
				...
				return response.blob().then( function (blob) {
					...
					return new Promise(function(resolve) {
						reader.addEventListener("loadend", function() {
							var pbf_data = pako.inflate(new Uint8Array(reader.result));

							var pbf = new index(pbf_data);

							var vectorTile = new VectorTile(pbf)

							if (Object.keys(vectorTile.layers).length > 0) {
								var tileDB = new iDB('tileDB');
								tileDB.setTile(thisVG.options.name, {
									coord: thisVG._tileCoordsToKey(coords),
...
idb.prototype = {
	connectDB: function(layer, f) {
		...
		request.onsuccess = function(){
			f(request.result, layer);
		}
		request.onupgradeneeded = function(e){
			for (i in layerGroups) {
				var group = layerGroups[i];
				if (group.name === 'Basemap') continue;
				for (j in group.layers) {
					e.currentTarget.result.createObjectStore(group.layers[j].options.name, { keyPath: "coord" });
				}
			}

			thisDB.connectDB(layer, f);
		}
	},

	getTile: function (coord, layer) {
		var thisDB = this;
		return new Promise (function(resolve, reject) {
			thisDB.connectDB(layer, function(db, layer) {
				if (!db.objectStoreNames.contains(layer)) {
					db.createObjectStore(layer, { keyPath: "coord" });
				}
				var request = db.transaction([layer], "readonly").objectStore(layer).get(coord);
	...
	setTile: function(layer,tile) {
		this.connectDB(layer, function(db, layer) {
			if (!db.objectStoreNames.contains(layer)) {
				db.createObjectStore(layer, { keyPath: "coord" });
			}
			var objStore = db.transaction([layer], "readwrite").objectStore(layer);
			// console.log(tile);
